...
// ==========================================
// BACKEND API FETCH HELPERS & REAL-TIME WEBSOCKET
// ==========================================
let socket = null;
...
async function loadStateFromBackend() {
  try {
    // Fetch Career Page settings
    try {
      const career = await apiFetch('/api/career');
      if (career) {
        document.getElementById('career-subdomain').value = career.domain || 'devasri-tech';
        document.getElementById('career-intro').value = career.description || 'Build the future of technology with us.';
        const statusLink = document.querySelector('.status-link');
        if (statusLink) {
          statusLink.textContent = `interviehire.com/careers/${career.domain || 'devasri-tech'} ↗`;
          statusLink.href = `https://interviehire.com/careers/${career.domain || 'devasri-tech'}`;
        }
      }
    } catch (e) {
      console.error("Failed to load career page from backend", e);
    }
    ...
    if (candidatesData) {
      AppState.candidates = candidatesData.map(c => {
        ...
        return {
          ...
          status: status,
          score: score,
          registeredOn: c.registered_on ? new Date(c.registered_on).toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' }) + ', 10:00 AM' : 'Recently'
        };
      });
    }
...
function updateSummaryMetrics() {
  ...
  if (stats) {
    ...
    resumeCount = stats.resume_analysed;
    screeningCount = stats.screening_attempted;
    functionalCount = stats.functional_attempted;
  } else {
  ...
  if (appPills.length >= 4) {
    appPills[0].textContent = stats ? stats.career_page : 0; // Career Page
    appPills[1].textContent = stats ? stats.bulk_upload : 0; // Bulk Upload
    appPills[2].textContent = stats ? stats.scheduled : screeningCount; // Scheduled screening
    appPills[3].textContent = stats ? stats.direct_link : (totalApplicants - screeningCount); // Direct Link / Rest
  }
  
  // Resume analysis card pills:
  const resPills = document.querySelectorAll('.card-metric:nth-child(2) .m-pill .v');
  if (resPills.length >= 3) {
    resPills[0].textContent = stats ? stats.resume_analysed : resumeCount; // Analysed
    resPills[1].textContent = stats ? stats.resume_shortlisted : 0; // Shortlisted
    resPills[2].textContent = stats ? stats.resume_waitlisted : 0; // Waitlisted
  }
  ...
  if (scrPills.length >= 4) {
    scrPills[0].textContent = stats ? stats.screening_attempted : (screeningCount > 0 ? screeningCount - 1 : 0); // Attempted
    scrPills[1].textContent = stats ? stats.screening_scheduled : (screeningCount > 0 ? 1 : 0); // Scheduled
    scrPills[2].textContent = stats ? stats.screening_shortlisted : 0; // Shortlisted
    scrPills[3].textContent = stats ? stats.screening_waitlisted : 0; // Waitlisted
  }
  ...
  if (funPills.length >= 4) {
    funPills[0].textContent = stats ? stats.functional_attempted : (functionalCount > 0 ? functionalCount - 1 : 0); // Attempted
    funPills[1].textContent = stats ? stats.functional_scheduled : (functionalCount > 0 ? 1 : 0); // Scheduled
    funPills[2].textContent = stats ? stats.functional_shortlisted : 0; // Shortlisted
    funPills[3].textContent = stats ? stats.functional_waitlisted : 0; // Waitlisted
  }
}
...
async function importCsvCandidates() {
  ...
  for (const cand of csvParsedCandidates) {
...
async function importResumesCandidates() {
  ...
  for (const file of uploadedFiles) {
    ...
    await addCandidateToAppState(rawName, email, phone, activeJob);
  }
...
async function addCandidateToAppState(name, email, phone, job) {
  try {
    ...
    showPremiumToast(`Failed to add candidate "${name}" to backend database.`, "error");
  }
}
...
async function updateCandidateStatus(candId, newStatus) {
  ...
  try {
    ...
    showPremiumToast("Failed to update candidate status: " + e.message, "error");
  }
}
...
function saveStateToLocalStorage() {
  // Persistence migrated to backend database
}

function loadStateFromLocalStorage() {
  // Loaded from backend database dynamically
}
