...
async function loadStateFromBackend() {
  try {
    ...
    try {
      ...
      if (org) {
        document.getElementById('career-subdomain').value = org.domain || 'devasri-tech';
        document.getElementById('career-intro').value = org.description || 'Build the future of technology with us.';
        const statusLink = document.querySelector('.status-link');
        if (statusLink) {
          statusLink.textContent = `interviehire.com/careers/${org.domain || 'devasri-tech'} ↗`;
          statusLink.href = `https://interviehire.com/careers/${org.domain || 'devasri-tech'}`;
        }
      }
    } catch (e) {
      console.error("Failed to load organisation from backend", e);
    }
    ...
    try {
      ...
      if (teamData && teamData.members) {
        ...
        // Update default profile name to first member or Org Admin
    ...
    try {
      ...
      console.error("Failed to load usage stats from backend", e);
      AppState.stats = null;
    }
    ...
    console.error("Critical error loading state from backend:", e);
...
function connectWebSocket() {
  ...
  const wsHost = window.location.host === 'localhost:3000' ? 'localhost:8000' : window.location.host;
...
async function syncJobParameters(job) {
  try {
    const topics = [
      {
    ...
    btn.addEventListener('click', async () => {
      ...
      if (member) {
        try {
          ...
          console.error("Failed to revoke member on backend:", e);
          showPremiumToast(e.message || "Failed to revoke member.", "error");
        }
      }
    });
  });
...
function applyDateRangeGlobally() {
  ...
  if (activeJob) {
    const jobCandidates = filterCandidatesByDateRange(
      AppState.candidates.filter(c => c.jobId === String(activeJob.id))
    );
...
function recalculateJobPipelines() {
  ...
  AppState.jobs.forEach(job => {
    const jobCandidates = dateFiltered.filter(c => c.jobId === String(job.id));
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (createJobForm) {
    createJobForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        showPremiumToast("Error creating job: " + (err.message || "communication error"), "error");
      }
    });
  }
  ...
  inviteMemberForm.addEventListener('submit', async (e) => {
    ...
    const usertypeInput = document.getElementById('member-role-input').value;
    const userType = usertypeInput === 'Org. Admin' ? 'org_admin' : 'member';
    ...
    try {
      ...
      if (response) {
        ...
        soundEngine.playChime([261.63, 392.00, 523.25], 0.2, 0.08);
        showPremiumToast(`Successfully invited team member "${name}".`, 'success');
      }
    } catch (e) {
      console.error("Failed to invite team member on backend:", e);
      showPremiumToast(e.message || "Failed to invite team member.", "error");
    }
  });

  // 3. Settings Forms (Mock updates with inline alerts)
  // 3. Settings Forms
  const careerForm = document.getElementById('career-settings-form');
  if (careerForm) {
    careerForm.addEventListener('submit', async (e) => {
      ...
      try {
        await apiFetch('/api/organisation', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        console.error("Failed to save career settings to backend:", err);
        showPremiumToast("Failed to save settings.", "error");
      }
    });
  }
  ...
  if (passForm) {
    passForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (alertBox) {
          alertBox.textContent = `❌ ${err.message || 'Verification failed.'}`;
          alertBox.style.color = 'var(--color-orange)';
          alertBox.style.display = 'block';
        }
      }
  ...
  // Continue button — process file or pasted text with backend extract-jd
  ...
  if (btnContinue) {
    btnContinue.addEventListener('click', async () => {
      ...
      const sourceName = createJobUploadedFileName || 'job_description.txt';
...
async function importResumesCandidates() {
  ...
  const btn = document.getElementById('btn-resumes-import');
  const origHTML = btn ? btn.innerHTML : '';
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = `<span class="ra-spinner" style="display:inline-block;width:12px;height:12px;border:2px solid rgba(255,255,255,0.3);border-top-color:#fff;border-radius:50%;animation:spin-mini 0.6s linear infinite;margin-right:6px;vertical-align:middle;"></span> Importing...`;
  }
  ...
  try {
    ...
    const response = await apiFetch(`/api/jobs/${activeJob.id}/applicants/upload-resumes`, {
      method: 'POST',
      body: formData
    });
    ...
    if (response) {
      await loadStateFromBackend();
      showPremiumToast(`Successfully processed and imported ${response.length} candidate(s) into "${activeJob.roleName}".`, "success");
      soundEngine.playChime([392.00, 523.25, 659.25], 0.2, 0.08);
    }
  } catch (err) {
    console.error("Failed to import resumes:", err);
    showPremiumToast("Failed to import resumes: " + err.message, "error");
  } finally {
    if (btn) {
      btn.disabled = false;
...
async function importManualQueue() {
  ...
  for (const cand of sourcingQueue) {
    await addCandidateToAppState(cand.name, cand.email, cand.phone, activeJob);
  }
...
async function addCandidateToAppState(name, email, phone, job) {
  try {
    const source = currentSourcingMode === 'analyse' ? 'bulk_upload' : 'direct_link';
    const response = await apiFetch(`/api/jobs/${job.id}/applicants`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: name,
    ...
    if (response) {
      ...
      let attemptedAtVal = null;
      let recScreeningVal = null;
      let recScreeningScoreVal = null;

      if (status === 'Screening') {
        attemptedAtVal = new Date().toISOString();
        recScreeningVal = 'Good fit';
        recScreeningScoreVal = 80.0;
        await apiFetch(`/api/jobs/applicants/${response.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            screening_status: 'scheduled',
            screening_score: 80.0,
            attempted_at: attemptedAtVal,
            recruiter_screening: recScreeningVal,
            recruiter_screening_score: recScreeningScoreVal
          })
        });
      }
      ...
      AppState.candidates.push({
        ...
        email: email,
        phone: phone || '—',
        source: source === 'bulk_upload' ? 'Bulk Upload' : 'Direct Link',
        jobApplied: job.roleName,
        jobId: String(job.id),
        status: status,
        score: score,
        registeredOn: dateStr,
        attemptedAt: formatAttemptedAt(attemptedAtVal),
        interviewStatus: status === 'Screening' ? 'Scheduled' : 'Pending',
        cheatProbability: null,
        interviewScore: status === 'Screening' ? 80.0 : null,
        recruiterScreening: recScreeningVal,
        recruiterScreeningScore: recScreeningScoreVal
      });
    }
  } catch (e) {
    console.error("Failed to add candidate to backend:", e);
    showPremiumToast(e.message || "Failed to add candidate.", "error");
  }
}
  ...
  Object.entries(cols).forEach(([stage, col]) => {
    ...
    col.addEventListener('drop', (e) => {
      ...
      if (candidate && candidate.status !== stage) {
        updateCandidateStatus(candidateId, stage);
      }
    });
  });
}
  ...
  document.querySelectorAll('.ra-data-table tr[data-cid]').forEach(row => {
    ...
    analyseBtn?.addEventListener('click', async () => {
      ...
      if (candidate && candidate.resumeUrl) {
        ...
        analyseBtn.innerHTML = `<span class="ra-spinner" style="display:inline-block;width:10px;height:10px;border:2px solid rgba(255,255,255,0.3);border-top-color:#fff;border-radius:50%;animation:spin-mini 0.6s linear infinite;margin-right:6px;vertical-align:middle;"></span> Fetching...`;
        
        try {
          const res = await apiFetch(`/api/jobs/applicants/${cid}/resume-text`);
          if (res && res.text) {
            resumeTextCache[cid] = res.text;
          } else {
            resumeTextCache[cid] = null;
          }
          await runResumeAnalysis(cid, job);
        } catch (err) {
          console.error("Auto analysis failed, falling back to file upload:", err);
          fileInput?.click();
          const handler = () => {
...
function renderJobDetailPanes(job) {
  ...
  const jobCandidates = filterCandidatesByDateRange(AppState.candidates).filter(c => {
    const matchesJob = c.jobId === String(job.id);
  ...
  if (pane) {
    ...
    const jobCands = AppState.candidates.filter(c => c.jobId === String(job.id));
...
async function updateCandidateStatus(candId, newStatus) {
  ...
  try {
    ...
    if (newStatus === 'Rejected') {
      ...
      body.remarks = '';
      body.functional_status = 'scheduled';
      body.functional_score = parseFloat(candidate.score.replace('%', '')) || 85.0;
    } else if (newStatus === 'Screening') {
      body.remarks = '';
      body.screening_status = 'scheduled';
      body.screening_score = parseFloat(candidate.score.replace('%', '')) || 80.0;
      body.functional_status = null;
    } else if (newStatus === 'Resume') {
      body.remarks = '';
      body.screening_status = null;
      body.functional_status = null;
      body.resume_analysed = true;
    }

    if ((newStatus === 'Screening' || newStatus === 'Functional') && candidate.interviewScore == null) {
      body.attempted_at = new Date().toISOString();
      body.recruiter_screening = 'Good fit';
      body.recruiter_screening_score = 90.0;
      body.cheat_probability = 'low';
    }

    await apiFetch(`/api/jobs/applicants/${candId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    candidate.status = newStatus;
    if (body.attempted_at) candidate.attemptedAt = formatAttemptedAt(body.attempted_at);
    if (body.recruiter_screening) candidate.recruiterScreening = body.recruiter_screening;
    if (body.recruiter_screening_score) candidate.recruiterScreeningScore = body.recruiter_screening_score;
    if (body.cheat_probability) {
      const cheatMap = { 'low': 'Low', 'medium': 'Medium', 'high': 'High' };
      candidate.cheatProbability = cheatMap[body.cheat_probability];
    }
    if (body.functional_score) candidate.interviewScore = body.functional_score;
    else if (body.screening_score) candidate.interviewScore = body.screening_score;
    ...
    showPremiumToast("Error updating candidate: " + err.message, "error");
  }
}
