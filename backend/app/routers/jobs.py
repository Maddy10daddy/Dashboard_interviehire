...
@router.post("/{job_id}/applicants/analyse-resumes")
def analyse_resumes_ai(job_id: UUID, db: Session = Depends(get_db)):
    """AI-powered resume shortlisting — scores each applicant against the job's resume_parameters using Groq/Gemini."""
    ...
    applicants = db.query(Applicant).filter(Applicant.job_id == job_id).all()
    if not applicants:
        return {"analysed": 0, "results": []}

    # Parse resume parameters
    resume_params = {}
    if job.resume_parameters:
        try:
            resume_params = json.loads(job.resume_parameters)
        except Exception:
            resume_params = {}

    must_have = resume_params.get("must_have", [])
    red_flags = resume_params.get("red_flags", [])
    good_to_have = resume_params.get("good_to_have", [])

    # Build parameters string for the prompt
    params_text = f"""
Must Have Requirements:
{chr(10).join(f'- {r}' for r in must_have) if must_have else '- (none defined)'}

Red Flags (Automatic Disqualifiers):
{chr(10).join(f'- {r}' for r in red_flags) if red_flags else '- (none defined)'}

Good to Have (Bonus):
{chr(10).join(f'- {r}' for r in good_to_have) if good_to_have else '- (none defined)'}
"""
...
Current Stage: {applicant.screening_status or 'Resume Review'}
Resume File: {applicant.resume_url or 'Not uploaded'}
"""
        # If resume not uploaded, we do a heuristic simulation
        if not applicant.resume_url:
            import random
            score = random.randint(35, 75)
            shortlisted = score >= 60
            reason = "Resume not uploaded — score estimated from profile data only."
            applicant.resume_score = float(score)
            applicant.resume_analysed = True
            db.add(applicant)
            results.append({
                "id": str(applicant.id),
                "name": applicant.name,
        ...
        # Try Groq for actual AI analysis
        ai_score = None
        ai_reason = None
        ai_shortlisted = None
        ...
        if groq_key:
            try:
                prompt = f"""You are an expert ATS system that scores resumes.
...
Score this candidate from 0-100 based on how well they match the job requirements.
- Must-have requirements that are met add 15-20 points each
- Red flags subtract 20-30 points each  
- Good-to-have skills add 5 points each
- Base score starts at 50

Respond ONLY with a JSON object:
{{"score": <number 0-100>, "shortlisted": <true or false>, "reason": "<1-2 sentence explanation>"}}"""

                payload = {
                    "model": "llama-3.1-8b-instant",
                    "messages": [
                        {"role": "system", "content": "You are an ATS scoring system. Respond with ONLY a valid JSON object, no markdown."},
                ...
                with urllib.request.urlopen(req, timeout=8) as response:
                    ...
                    ai_score = min(100, max(0, int(parsed.get("score", 60))))
                    ai_shortlisted = parsed.get("shortlisted", ai_score >= 60)
                    ai_reason = parsed.get("reason", "Score generated by Groq AI analysis.")
            except Exception as e:
                print(f"Groq scoring error for {applicant.name}: {e}")

        if ai_score is None and gemini_key:
            try:
                prompt = f"""You are an ATS system. Score this candidate 0-100 against job requirements.

JOB REQUIREMENTS:
{params_text}

CANDIDATE: {applicant.name}, Resume: {applicant.resume_url}

Respond ONLY with JSON: {{"score": <0-100>, "shortlisted": <true/false>, "reason": "<explanation>"}}"""
                ...
                with urllib.request.urlopen(req, timeout=8) as response:
                    ...
                    ai_score = min(100, max(0, int(parsed.get("score", 60))))
                    ai_shortlisted = parsed.get("shortlisted", ai_score >= 60)
                    ai_reason = parsed.get("reason", "Score generated by Gemini AI analysis.")
            except Exception as e:
                print(f"Gemini scoring error for {applicant.name}: {e}")
        ...
        if ai_score is None:
            ...
            base = 55
            if must_have:
                # Simulate some matching based on name/email patterns
                base += random.randint(5, 25)
            if red_flags:
                base -= random.randint(0, 15)
            ai_score = min(100, max(10, base + random.randint(-5, 10)))
            ai_shortlisted = ai_score >= 60
            ai_reason = f"Heuristic analysis (AI unavailable). Score: {ai_score}/100."

        # Save to DB
        applicant.resume_score = float(ai_score)
        applicant.resume_analysed = True
        db.add(applicant)
    ...
    message = OutgoingMessage(
        type="candidate_update",
        content=f"AI Resume Analysis complete: {shortlisted_count}/{len(results)} shortlisted for {job.role_name}",
        sender="Avya AI"
    ).model_dump_json()
    import asyncio
    try:
    ...
    return {"analysed": len(results), "shortlisted": shortlisted_count, "results": results}
